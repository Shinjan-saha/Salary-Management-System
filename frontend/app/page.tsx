...
export default function EmployeeDashboard() {
  ...
  const fetchEmployees = async () => {
    ...
    try {
      ...
      // Normalize the data to handle both 'id' and 'ID' fields
      const normalizedEmployees = res.data.map((emp: any) => ({
        ...emp,
        id: emp.id || emp.ID, 
      }));
      
      console.log("Fetched employees:", normalizedEmployees); // Debug log
  ...
  const deleteEmployee = async (id: number | undefined) => {
    ...
    try {
      ...
      if (err.response) {
        ...
        console.error("Attempted URL:", `${api.defaults.baseURL}/employee/${id}`);
      }
      
      alert(`Failed to delete employee. Error: ${err.response?.status || 'Network Error'}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const editEmployee = (employee: Employee) => {
    ...
    if (!employeeId) {
      alert("Employee ID not found. Cannot edit.");
      return;
    }
  ...
  const paySalary = async (id: number | undefined) => {
    if (!id) {
      alert("Employee ID not found. Cannot process payment.");
      return;
    }

    console.log("Processing payment for employee ID:", id); // Debug log

    setLoading(true);
    try {
      const res = await api.post("/employee/pay", { employee_id: id });
      alert(`Payment initiated successfully!\nOrder ID: ${res.data.order_id}\nAmount: ₹${res.data.amount}\nCurrency: ${res.data.currency}`);
      await fetchEmployees(); // Refresh to update payment status
    } catch (err) {
